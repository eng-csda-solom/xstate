...
const CategorizationList = ({ selectedCategory, elements, data, depth, onSelect, subcategoriesClassName, groupClassName, t, ajv, config, }) => {
    const filteredElements = useMemo(() => {
        return elements.filter((category) => isVisible(category, data, undefined, ajv, config));
    ...
    return (React.createElement("ul", { className: subcategoriesClassName }, filteredElements.map((category, idx) => {
        if (isCategorization(category)) {
            return (React.createElement("li", { key: categoryLabels[idx], className: groupClassName },
                ...
                React.createElement(CategorizationList, { selectedCategory: selectedCategory, elements: category.elements, data: data, ajv: ajv, config: config, depth: depth + 1, onSelect: onSelect, subcategoriesClassName: subcategoriesClassName, groupClassName: groupClassName, t: t })));
        }
        else {
...
const CategorizationRenderer = ({ data, uischema, schema, path, selected, t, visible, getStyleAsClassName, onChange, ajv, config, }) => {
    ...
    return (React.createElement("div", { className: classNames, hidden: visible === null || visible === undefined ? false : !visible },
        React.createElement("div", { className: masterClassNames },
            React.createElement(CategorizationList, { elements: elements, selectedCategory: elements[safeCategory], data: data, ajv: ajv, config: config, depth: 0, onSelect: onCategorySelected, subcategoriesClassName: subcategoriesClassName, groupClassName: groupClassName, t: t })),
